...
prompt_email_personalization, config_email_personalization = get_prompt_config(prompt_tag="prompt_email_personalization_persona", label="stage_v1")
...
class EmailPersonalizationTool(BaseTool):
    ...
    def personalization_email(self, problem_statement, personas, persona_name_list, conversation_id, user_id):
        llm = ChatOpenAI(model=config_email_personalization['model'], temperature=config_email_personalization['temperature'], api_key=OPENAI_API_KEY)
    ...
    def email_llm(self, problem_statement_, persona_summary_str_, persona_names_, conversation_id, user_id):
        max_retries = 3

        persona_name_list, Incentive_list, Call_to_Action_list, Personalized_Content_list = [], [], [], []
        for attempt in range(max_retries):
            email_dict = self.personalization_email(problem_statement=problem_statement_, personas=persona_summary_str_,
    ...
    async def save_audience_email_template(self, 
                                user_id: str,
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        try:
            ...
            conversation_stage = parameters.get("conversation_stage", "creating_email_personalization")
            ...
            if missing_params:
                ...
                #     conversation_stage = "collecting_persona_name"
                    ...
                    response_message = "To create personalized email content, I need a bit more information. " + ", ".join(missing_params) + " would be helpful."
            ...
            if personalization_data_type.lower() == "persona data":
                ...
                if persona_summary_list:
                    ...
                    for persona in persona_summary_list:
                        ...
                        if persona_name.lower() == 'all':
                            ...
                            persona_name_list, Incentive_list, Call_to_Action_list, Personalized_Content_list = self.email_llm(problem_statement, summary, name, conversation_id, user_id)
                ...
                email_template = create_genralize_email_template(campaign_context=all_filter_criteria, openai_key=OPENAI_API_KEY, problem_statement=problem_statement)
                ...
                sample_subject = subject_lines[0] if subject_lines else "Personalized Offer Just For You!"
...
"""
            ...
            logger.error(f"Error executing EmailPersonalizationTool: {str(e)}", exc_info=True)
            return ToolResponse(
                status="error",
    ...
    async def store_personalization_data_type(self, doc_id: str, personalization_data_type: str) -> bool:
        ...
        try:
            ...
            if result.modified_count > 0:
                logger.info(f"Updated email personalization {doc_id} with personalization_data_type={personalization_data_type}")
                return True
            else:
            ...
            logger.error(f"Error updating email personalization with personalization data type: {str(e)}")
            return False
