...
class JourneyTool(BaseTool):
    ...
    async def _create_journey(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        if "problem_statement" in parameters and "user_problem_statement" not in parameters:
        ...
        if "has_audience_model" in parameters:
            if parameters['has_audience_model'] is None or parameters['has_audience_model'] == "":
                return ToolResponse(
                    status="error",
                    message="Do you have a model for the audience?",
                    required_inputs=["has_audience_model"],
        ...
        required_params = ["user_problem_statement", "budget", "target_id", "session_token", "name", "date"]
        ...
        if missing:
            ...
            if "model_id" in missing and "model_id" in required_params:
                ...
                tech_params = [param for param in ["budget", "name"] if param in missing]
                return ToolResponse(
                    status="error",
    ...
    async def _get_journey_status(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        try:
            ...
            if journey_status.lower() == "not_started" or journey_status.lower() == "processing":
                message = f"🌍✨ Your journey is taking shape! Please check back in a few minutes"
            elif journey_status.lower() == "completed":
                message = f"✅ Journey Completed! You can check it from the journey page"
    ...
    async def _check_journey_report(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        # if True:
            ...
            # Handle document_content if provided (for uploads via process endpoint)
                ...
                filename = f"journey_{parameters['journey_id']}_{timestamp_str}_modified.docx"
            ...
            if "uploaded_document" in parameters and parameters["uploaded_document"]:
                message = "Your modified journey document has been processed successfully."
            elif "document_content" in parameters and parameters["document_content"]:
                message = "Your modified journey document has been processed successfully."
            else:
                message = "Journey report processed successfully with the existing document."
    ...
    def _generate_journey_doc(self, report_data: Dict[str, Any], journey_id: str) -> Dict[str, Any]:
        """
    ...
    async def _download_journey_document(self, parameters: Dict[str, Any]) -> ToolResponse:
        """
    ...
    async def _upload_journey_document(self, parameters: Dict[str, Any]) -> ToolResponse:
        """
