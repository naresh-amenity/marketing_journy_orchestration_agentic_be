...
prompt_digitalad_personalization, config_digitalad_personalization = get_prompt_config(
    prompt_tag="prompt_digitalad_personalization_persona", label="stage_v1"
)
...
class DigitalAdPersonalizationSchema(BaseModel):
    ...
    Headlines: List[list] = Field(
        description="headlines for each persona in nested list format"
    )
...
class DigitalAdPersonalizationTool(BaseTool):
    ...
    def personalization_digitalad(
        self, problem_statement, personas, persona_name_list, conversation_id, user_id
    ):
        llm = ChatOpenAI(
            model=config_digitalad_personalization["model"],
            temperature=config_digitalad_personalization["temperature"],
        ...
        config = config_llm_callback(
            run_name="digitalad_personalization_persona",
            tag="digitalad_personalization_persona",
    ...
    def digitalad_llm(
        ...
        (
            ...
            Recommended_Platforms_list,
        ) = ([], [], [], [], [], [])
        for attempt in range(max_retries):
            digitalad_dict = self.personalization_digitalad(
    ...
    async def save_audience_digitalad_template(
        self,
        user_id: str,
    ...
    async def store_personalization_data_type(
        ...
        try:
            ...
            if result.modified_count > 0:
                logger.info(
                    f"Updated digital ad personalization {doc_id} with personalization_data_type={personalization_data_type}"
                )
                return True
            else:
            ...
            logger.error(
                f"Error updating digital ad personalization with personalization data type: {str(e)}"
            )
            return False
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        try:
            ...
            conversation_stage = parameters.get(
                "conversation_stage", "creating_digitalad_personalization"
            )
            ...
            if missing_params:
                ...
                #     conversation_stage = "collecting_persona_name"
                    ...
                    response_message = (
                        "To create personalized digital ad content, I need a bit more information. "
                        + ", ".join(missing_params)
                        + " would be helpful."
                    )
            ...
            if personalization_data_type.lower() == "persona data":
                ...
                for persona in persona_summary_list:
                    ...
                    if persona_name.lower() == "all":
                        ...
                        (
                            ...
                            problem_statement, summary, name, conversation_id, user_id
                        )
                        for (
                            incentive,
                            headlines,
                ...
                for ad in digitalad_personalization:
                    try:
                        ...
                        logger.error(
                            f"Error saving digital ad personalization: {str(e)}"
                        )
                ...
                return ToolResponse(
                    ...
                    data={"digitalad_personalization": digitalad_personalization},
                )
                ...
                digitalad_template = create_genralize_digitalad_template(
...
"""
            ...
            logger.error(
                f"Error executing DigitalAdPersonalizationTool: {str(e)}", exc_info=True
            )
            return ToolResponse(
                status="error",
