...
prompt_direct_mail_personalization, config_direct_mail_personalization = get_prompt_config(prompt_tag="prompt_direct_mail_personalization_persona", label="stage_v1")
...
class DirectMailPersonalizationTool(BaseTool):
    ...
    def personalization_direct_mail(self, problem_statement, persona_details, persona_name_list, conversation_id, user_id):
        llm = ChatOpenAI(model=config_direct_mail_personalization['model'], temperature=config_direct_mail_personalization['temperature'], api_key=OPENAI_API_KEY)
        ...
        config = config_llm_callback(run_name="directmail_personalization_persona", tag="directmail_personalization_persona", conversation_id=conversation_id, user_id=user_id)

        output = chain.invoke(
            {"problem_statement": problem_statement, "persona_details": persona_details, "persona_name": persona_name_list}, config=config)

        return output

    def direct_mail_llm(self, problem_statement_, persona_details_, persona_names_, conversation_id, user_id):
        max_retries = 3

        persona_name_list, Incentive_list, Call_to_Action_list, Personalized_Content_list = [], [], [], []
        for attempt in range(max_retries):
            direct_mail_dict = self.personalization_direct_mail(problem_statement=problem_statement_,
            ...
            len_list = [len_persona_name_list, len_Incentive_list, len_Call_to_Action_list, len_Personalized_Content_list]

            set_len_list = list(set(len_list))

            if len(set(set_len_list)) == 1:
                return persona_name_list, Incentive_list, Call_to_Action_list, Personalized_Content_list
    ...
    async def store_personalization_data_type(self, doc_id: str, personalization_data_type: str) -> bool:
        ...
        try:
            ...
            if result.modified_count > 0:
                logger.info(f"Updated direct mail personalization {doc_id} with personalization_data_type={personalization_data_type}")
                return True
            else:
            ...
            logger.error(f"Error updating direct mail personalization with personalization data type: {str(e)}")
            return False
    
    async def save_audience_directmail_template(self, 
                                  user_id: str,
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        try:
            ...
            # Log the parameters
            logger.info(f"Executing DirectMailPersonalizationTool with parameters: user_id={user_id}, personalization_data_type={personalization_data_type}")
            ...
            if missing_params:
                ...
                #     conversation_stage = "collecting_persona_name"
                    ...
                    response_message = "To create personalized direct mail content, I need a bit more information. " + ", ".join(missing_params) + " would be helpful."
                ...
                if personalization_data_type.lower() == "persona data":
                    ...
                    for persona in persona_summary_list:
                        ...
                        if not name or not summary:
                            logger.warning(f"Skipping persona with missing name or summary: {persona}")
                            continue
                    ...
                    if not matching_personas:
                        logger.warning(f"No matching personas found for persona_name={persona_name}")
                        return ToolResponse(
                            status="error",
                    ...
                    logger.info(f"Processing {len(matching_personas)} matching personas")
                    ...
                    for persona_data in matching_personas:
                        ...
                        try:
                            logger.info(f"Generating direct mail personalization for persona: {name}")
                            persona_name_list, Incentive_list, Call_to_Action_list, Personalized_Content_list = self.direct_mail_llm(problem_statement, summary, name, conversation_id, user_id)
                            
                            # Validate results
                            if not persona_name_list or not Incentive_list or not Call_to_Action_list or not Personalized_Content_list:
                                logger.warning(f"Incomplete results for persona {name}")
                                continue
                                
                            for incentive, call_to_action, personalized_content in zip(Incentive_list, Call_to_Action_list, Personalized_Content_list):
                            ...
                            logger.error(f"Error processing persona {name}: {str(e)}", exc_info=True)
                    ...
                    for ep in direct_mail_personalization:
                        try:
                            ...
                            logger.error(f"Error saving direct mail personalization: {str(e)}", exc_info=True)
                    ...
                    try:
                        await self.db.save_tool_execution(
                            ...
                            result={"direct_mail_personalization": direct_mail_personalization},
                            user_id=user_id,
                            conversation_id=conversation_id
                        )
                    except Exception as e:
                        logger.error(f"Error saving tool execution: {str(e)}", exc_info=True)
                    ...
                    directmail_template = create_genralize_directemail_template(campaign_context=all_filter_criteria, openai_key=OPENAI_API_KEY, problem_statement=problem_statement)
                    ...
                    template_preview = directmail_template.get("directmail_template", "")
                ...
                I've created a personalized direct mail template based on your audience data."""
                    ...
                    direct_mail_personalization = [{
                        ...
                        "headlines": directmail_template.get("headlines", []),
                        "call_to_action_options": directmail_template.get("call_to_action_options", []),
                        "personalized_content": directmail_template.get("directmail_template", ""),
                        "personalization_strategy": directmail_template.get("personalization_strategy", {})
                    }]
            ...
            logger.error(f"Error executing DirectMailPersonalizationTool: {str(e)}", exc_info=True)
            return ToolResponse(
                status="error",
