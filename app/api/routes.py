...
from app.models.model import UserRequest, ApiResponse, ToolInput, ChatMessage, ConversationState
...
async def process_request(request: UserRequest, req: Request):
    ...
    try:
        ...
        if request.is_conversation_end:
            logger.info(f"Frontend requested to end conversation: {request.conversation_id} with name: {request.conversation_name or 'General Conversation'}")
...
async def get_user_conversations(user_id: str, req: Request, limit: int = 10):
    ...
    try:
        ...
        # Get conversations
        conversations = await mongodb.get_user_conversations(user_id=user_id, limit=limit)
...
async def get_conversation(conversation_id: str, user_id: str, req: Request, include_messages: bool = True):
    """
...
async def get_user_history(user_id: str, req: Request, limit: int = 0, conversation_id: str = None):
    """
...
async def update_conversation_name(
    ...
    try:
        ...
        if mark_as_ended:
            ...
            if success:
                return {"status": "success", "message": f"Conversation marked as ended with name: {conversation_name}"}
            else:
            ...
            if success:
                ...
                raise HTTPException(status_code=500, detail="Failed to update conversation name")
...
async def get_user_history(user_id: str, req: Request, model_id: str = ""):
    ...
    try:
        ...
        # Get conversations
        conversations = await mongodb.get_persona_summaries(user_id=user_id, model_id=model_id, conversation_id="", conversation_status=True)
...
async def get_generalized_personalization(user_id, db, audience_id) -> List[Dict[str, Any]]:
        """
