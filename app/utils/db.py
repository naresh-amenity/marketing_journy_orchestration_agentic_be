...
class MongoDB:
    ...
    async def create_conversation(
        ...
        doc = {
            ...
            "updated_at": now,
            "messages": [
                {"role": "user", "content": initial_message, "timestamp": now}
            ],
            "context": {},
    ...
    async def update_query_context(
        ...
        if not conversation:
            logger.error(
                f"Conversation not found for user_id={user_id} and conversation_id={conversation_id}"
            )
            return None
    ...
    async def get_missing_context_fields(
        self, user_id: str, conversation_id: str, expected_keys: List[str]
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    async def add_message_to_conversation(
        self, conversation_id: str, role: str, content: str, problem_statement=False
    ) -> bool:
        """
    ...
    async def set_conversation_model_id(
        self, conversation_id: str, model_id: str
    ) -> bool:
        """
    ...
    async def add_persona_narrative(
    ...
    async def update_conversation_name(
        ...
        # Update the document
        now = datetime.utcnow().isoformat()
        update_data = {"conversation_name": conversation_name, "updated_at": now}
        ...
        # Update the document
        result = await collection.update_one(
            {"conversation_id": conversation_id}, {"$set": update_data}
        )
    ...
    async def mark_conversation_ended(
        ...
        # Prepare update data
        now = datetime.utcnow().isoformat()
        update_data = {"is_ended": True, "updated_at": now}
    ...
    async def save_persona_summary(
        ...
        try:
            ...
            logger.error(
                f"Error while setting conversation_status to false. Error: {e}"
            )
    ...
    async def save_parameters_to_context(
        self, user_id: str, conversation_id: str, new_parameters: Dict[str, Any]
    ):
        """
        ...
        if existing:
            ...
            await collection.update_one(
                query, {"$set": {"parameters": updated_parameters}}
            )
        else:
            filtered_parameters = {
                k: v for k, v in new_parameters.items() if v not in [None, ""]
            }
            doc = {
                "user_id": user_id,
                "conversation_id": conversation_id,
    ...
    async def get_parameters_from_context(
    ...
    async def get_persona_summaries(
        self,
        user_id: str,
        model_id: str,
        conversation_id: str,
    ...
    async def get_email_personalizations(
        self,
        user_id: str,
    ...
    async def save_direct_mail_personalization(
        self,
        user_id: str,
        model_id: str,
        persona_name: str,
    ...
    async def get_direct_mail_personalizations(
        self,
        user_id: str,
        model_id: Optional[str] = None,
        conversation_id: Optional[str] = None,
    ...
    async def delete_persona_summaries(self, user_id: str, model_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Deleted {result.deleted_count} persona summaries for user_id={user_id}, model_id={model_id}"
            )
            return result.acknowledged
        except Exception as e:
    ...
    async def save_digitalad_personalization(
        self,
        user_id: str,
        model_id: str,
        persona_name: str,
    ...
    async def get_digitalad_personalizations(
        self,
        user_id: str,
        model_id: Optional[str] = None,
        conversation_id: Optional[str] = None,
    ...
    async def save_generalized_personalization(
        self,
        user_id: str,
        audience_id: str,
