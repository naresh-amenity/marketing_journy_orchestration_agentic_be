...
from app.models.model import UserRequest, ToolResponse, ApiResponse
...
TOOL_PARAM_DICT = {
    ...
    "email_personalization": ["user_id", "model_id", "personalization_data_type", "problem_statement", "persona_name", "audience_id"],
    "directmail_personalization": ["user_id", "model_id", "personalization_data_type", "problem_statement", "persona_name", "audience_id"],
    "digitalad_personalization": ["user_id", "model_id", "personalization_data_type", "problem_statement", "persona_name", "audience_id"],
    # "show_persona_summary": ["user_id", "model_id", "persona_name"],
    "persona_explain": ["user_id", "model_id"],
    "journy_tool": ["user_problem_statement", "budget", "target_id", "session_token", "name", "date", "persona_files", "model_id"]
}

TOOL_SELECTION_PROMPT, config_tool_selection = get_prompt_config(prompt_tag="tool selection prompt", label="stage_v2")
extraction_prompt, config_journy_action = get_prompt_config(prompt_tag="extraction prompt", label="stage_v1")
...
class ProblemStatementAnalysis(BaseModel):
    valid: bool = Field(description="Indicates whether the problem statement is complete and valid.")
    missing_fields: List[str] = Field(default_factory=list, description="List of fields that are missing or incomplete.")
...
class MainAgent:
    ...
    def __init__(self, mongodb: Optional[MongoDB] = None):
        ...
        self.tool_selection_chain = self.tool_selection_prompt | self.llm_tool_selection | self.parser
        #=================end tool selection======================================================
        ...
        self.journy_chain_prompt = PromptTemplate(
            ...
            partial_variables={"format_instructions": self.parser_journy.get_format_instructions()},
        )
        self.journy_chain = self.journy_chain_prompt | self.llm_journy_action | self.parser_journy
    ...
    def get_context_msg(self, query, conversation_history: List[Dict[str, Any]] = None):
        ...
        if conversation_history and len(conversation_history) > 0:
            ...
            last_6_messages = conversation_history[-50:] if len(conversation_history) >= 50 else conversation_history
    ...
    async def select_tool(self, query: str, conversation_history: List[Dict[str, Any]] = None, conversation_id: str = None, user: str = None) -> ToolSelection:
        """
        ...
        try:
            ...
            if conversation_history and len(conversation_history) > 0:
                ...
                for i, message in enumerate(last_6_messages):
                    ...
                    if role == "user" and any(phrase in content for phrase in [
                        "persona narrative", "create persona", "generate persona", 
                        "persona creation", "let's do persona", "make a persona"
                    ]):
                    ...
                    if role == "assistant" and any(phrase in content for phrase in [
                        "model id", "provide your model id", "what is your model id", 
                        "which model id", "need to know which model"
                    ]):
                        model_id_requested = True
                        logger.info("Assistant requested model_id")
                    
                    # Check if user provided model_id after being asked
                    if role == "user" and model_id_requested and i > 0 and last_6_messages[i-1].get("role") == "assistant":
                        # Try to extract model_id using regex
                        model_id_match = re.search(r'model[\s_]?id[:\s]*([^\s,\.]+)|model[:\s]*([^\s,\.]+)', content)
                        if model_id_match:
                            model_id_provided = model_id_match.group(1) or model_id_match.group(2)
                            logger.info(f"User provided model_id: {model_id_provided}")
                        elif content.strip() and not (
                            "personalization" in content or 
                            "journey" in content or 
                            "audience" in content or
                            "summary" in content
                        ):
                            ...
                            logger.info(f"Assuming user content is model_id: {model_id_provided}")
                    
                    # Check for last assistant message
                    if role == "assistant":
                        last_assistant_message = content
                        # Check if this message indicates successful persona creation
                        if "successfully created the persona narrative" in content and "would you like to create personalized content" in content:
                            persona_creation_completed = True
                
                # Look for parameter values mentioned in previous messages
                parameter_values = self._extract_parameters_from_history(last_6_messages)
            
            # If the query is empty but user wanted persona narrative and provided model_id, 
            # we should create the persona narrative
            if (not query or query.strip() == "") and wants_persona_narrative and model_id_requested and model_id_provided:
                logger.info("Empty query with persona narrative intent and model_id provided. Creating persona narrative.")
            ...
            if any(phrase in query.lower() for phrase in ["persona data", "use persona", "with persona", "using persona"]):
                personalization_data_type = "persona data"
            elif any(phrase in query.lower() for phrase in ["audience data", "use audience", "with audience", "using audience"]):
            ...
            if any(phrase in query.lower() for phrase in [
                "personalization", "personalized content", "personalize", 
                "create personalized", "make personalized", "do personalization"
            ]) and not any(specific_type in query.lower() for specific_type in [
                "email", "mail", "direct mail", "directmail", "digital ad", "digitalad", "ad"
            ]):
                is_general_personalization = True
                logger.info("Detected general personalization request without specific type")
            ...
            config = config_llm_callback(run_name="agent_tool_selection", tag="tool_selection", conversation_id=conversation_id, user_id=user)
            # Run the tool selection chain
            selection_dict = self.tool_selection_chain.invoke({"query": query, "user_history": context}, config=config)
            ...
            tool_selection = ToolSelection(
                ...
                conversation_stage=selection_dict.get("conversation_stage", "initial_greeting"),
                suggested_next_message=selection_dict.get("suggested_next_message", "How can I help you with personalization today?")
            )
            ...
            if model_id_provided and "model_id" not in tool_selection.parameters:
                ...
                logger.info(f"Added previously provided model_id={model_id_provided} to parameters")
            ...
            if (not query or query.strip() == "") and model_id_provided and model_id_requested:
                ...
                if wants_persona_narrative:
                    ...
                    if "model_id" not in tool_selection.parameters:
                        tool_selection.parameters["model_id"] = model_id_provided
                    logger.info("Forcing persona_summary tool for empty query after model_id was provided")
            
            # Explicitly add personalization_data_type if present in the query but not detected by LLM
            if personalization_data_type and "personalization" in tool_selection.tool_name:
                # Add personalization_data_type to parameters
                tool_selection.parameters["personalization_data_type"] = personalization_data_type
                logger.info(f"Added personalization_data_type={personalization_data_type} from query")
                ...
                if "personalization_data_type" in tool_selection.required_parameters:
                    tool_selection.required_parameters.remove("personalization_data_type")
                ...
                if personalization_data_type == "persona data":
                    ...
                    if "model_id" not in tool_selection.parameters and "model_id" not in tool_selection.required_parameters:
                    ...
                    if "audience_id" not in tool_selection.parameters and "audience_id" not in tool_selection.required_parameters:
            ...
            if persona_creation_completed and query.lower().strip() in ["yes", "yes please", "sure", "let's do it", "ok", "okay", "let's create personalized content"]:
                ...
                logger.info(f"Overriding tool selection to {personalization_type} based on post-persona creation context")
    ...
    def _extract_parameters_from_history(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        param_patterns = {
            "personalization_data_type": [r"using (persona|audience) data", r"with (persona|audience) data", r"(persona|audience) data"],
    ...
    def _fill_empty_parameters(self, current_params: Dict[str, Any], history_params: Dict[str, Any], tool_name: str) -> Dict[str, Any]:
        """
        ...
        for param in relevant_params:
            ...
            if (param not in updated_params or not updated_params[param]) and param in history_params:
                updated_params[param] = history_params[param]
                logger.info(f"Filled empty parameter {param}={history_params[param]} from conversation history")
    ...
    async def check_missing_parameters(self, tool_name: str, parameters: Dict[str, Any], user_id: str, conversation_id: Optional[str] = None) -> List[str]:
        """
        ...
        if tool_name in ["persona_summary"]:
            print("personalization_data_type gggggggggg66666666666666666666666", parameters.get("personalization_data_type", ""))
        ...
        personalization_tools = ["email_personalization", "directmail_personalization", "digitalad_personalization"]
        if tool_name in personalization_tools:
            ...
            if personalization_data_type == "audience data":
                ...
                logger.info(f"Using audience data - model_id and persona_name are optional")
        ...
        if tool_name == "journy_tool" and "action" in parameters:
            ...
            if action in ["get_journey_status", "get_journey_report", "check_journey_report"]:
                # Check if we're missing either journey_id or mongo_id based on the action
                journey_id_needed = action in ["get_journey_report", "check_journey_report"] and "journey_id" not in parameters
                mongo_id_needed = action == "get_journey_status" and "mongo_id" not in parameters
                
                # If we need either ID and have conversation context, try to retrieve from context
                if (journey_id_needed or mongo_id_needed) and conversation_id and user_id and self.mongodb:
                    try:
                        ...
                        if hasattr(self.mongodb, 'get_conversation'):
                            ...
                            conversation = await self.mongodb.get_conversation(conversation_id=conversation_id)
                            ...
                            if conversation and "context" in conversation:
                                ...
                                if journey_id_needed and "journey_id" in context:
                                    ...
                                    logger.info(f"Retrieved journey_id={context['journey_id']} from context")
                                
                                if mongo_id_needed and "mongo_id" in context:
                                    parameters["mongo_id"] = context["mongo_id"]
                                    logger.info(f"Retrieved mongo_id={context['mongo_id']} from context")
                    except Exception as e:
                        logger.error(f"Error retrieving journey IDs from context: {str(e)}")
        
        # Check each required parameter
        for param in required_params:
            if param not in parameters or not parameters[param] or (param == "persona_name" and isinstance(parameters[param], str) and parameters[param].strip() == ""):
                ...
                if self.mongodb and user_id and conversation_id:
                    try:
                        ...
                        if hasattr(self.mongodb, 'get_context_value'):
                            context_value = await self.mongodb.get_context_value(user_id, conversation_id, param)
                            if context_value:
                        ...
                        # If get_context_value doesn't exist, try to get from conversation history 
                            ...
                            logger.info(f"get_context_value method not found, trying to find {param} in conversation history")
                            history = await self.mongodb.get_conversation_history(conversation_id=conversation_id, limit=0)
                            # Simple extraction logic - this can be improved
                            param_value = None
                            for message in reversed(history):
                                content = message.get('content', '')
                                # Very basic extraction - looking for param=value pattern
                                if f"{param}=" in content or f"{param}:" in content or f"{param} =" in content or f"{param} :" in content:
                        ...
                        logger.error(f"Error retrieving context value for {param}: {str(e)}")
                        missing_params.append(param)
                else:
                    missing_params.append(param)
    ...
    def analize_problem_statment(self, problem_statment, tool_name, conversation_id, user):
        """
        Analyze the problem statement and return the appropriate action
        """
        prompt = langfuse.get_prompt("problem statment analysis personalization", label="stage_v1")
        config_problem_analysis_personalization = prompt.config
        PROMPT_PROBLEM_STATEMENT_ANALYSIS_PERSONALIZATION = prompt.compile()
        
        prompt = langfuse.get_prompt("problem statment analysis journey", label="stage_v1")
        ...
        config = config_llm_callback(run_name="problem_statment_analysis", tag=tag, conversation_id=conversation_id, user_id=user)
        output = chain.invoke(
            {"problem_statment": problem_statment}, config=config)
        return output
    ...
    async def process_request(self, request: UserRequest) -> ApiResponse:
        ...
        try:
            ...
            if request.problem_statement or request.model_id or request.audience_id or request.target_id:
            ...
            if request.user_id and conversation_id and self.mongodb:
                ...
                if conversation and conversation.get("user_id") == request.user_id:
                    ...
                    print("conversation_history, conversation_history", conversation_history)
            ...
            tool_selection = await self.select_tool(request.query, conversation_history, conversation_id, request.user_id)
            ...
            exsting_parm = await self.mongodb.get_parameters_from_context(request.user_id, conversation_id_)
            ...
            if request.problem_statement:
                if request.problem_statement:
                    analize_problem_statment_out = self.analize_problem_statment(request.problem_statement, request.last_tool, conversation_id, request.user_id)
                    if analize_problem_statment_out['valid']:
                        ...
                        print("INVALID PROBLEM STATEMENT:", analize_problem_statment_out['missing_fields'])
                        print("SUGGESTED IMPROVEMENTS:", analize_problem_statment_out['suggestions'])
                        return ApiResponse(
                        status="problem_statment_invalid",
                        message="Please provide a valid problem statement",
                        data = {"missing_fields": analize_problem_statment_out['missing_fields'], 
                                "suggestions": analize_problem_statment_out['suggestions']},
            ...
            if exsting_parm.get("problem_statement"):
                ...
                print("PROBLEM STATEMENT SET FROM CONTEXT:", exsting_parm.get("problem_statement"))
            ...
            if exsting_parm.get("persona_name"):
                ...
                print("PERSONA NAME SET FROM CONTEXT:", exsting_parm.get("persona_name"))
            ...
            if exsting_parm.get("has_audience_model"):
                parameters["has_audience_model"] = exsting_parm.get("has_audience_model")
                print("has_audience_model FROM CONTEXT:", exsting_parm.get("has_audience_model"))
            
            print("PARAMETERS:", parameters)
            parameters_copy = parameters.copy()
            if "action" in parameters_copy:
                parameters_copy.pop('action')
            await self.mongodb.save_parameters_to_context(request.user_id, conversation_id_, parameters_copy)
            ...
            if tool_selection.tool_name == "journy_tool":
                ...
                if "problem_statement" in request.__dict__ and request.problem_statement:
                    parameters["user_problem_statement"] = request.problem_statement
                    logger.info(f"Using problem_statement from request for user_problem_statement: {request.problem_statement[:30]}...")
                
                # If action is missing, try to extract it from the query
                if "action" not in parameters:
                    get_context_msg = self.get_context_msg(request.query, conversation_history)
                    config = config_llm_callback(run_name="journey_action_detection", tag="journey_action_detection", conversation_id=conversation_id, user_id=request.user_id)
                    action = await self.journy_chain.ainvoke({"query": get_context_msg}, config=config)
            ...
            if conversation_id:
                ...
                logger.info(f"Added conversation_id={conversation_id} to tool parameters")
            ...
            if tool_selection.tool_name in personalization_tools:
                ...
                if not personalization_data_type:
                    ...
                    if request.query:
                        if any(phrase in request.query.lower() for phrase in ["persona data", "use persona", "with persona"]):
                            personalization_data_type = "persona data"
                            parameters["personalization_data_type"] = "persona data"
                            logger.info(f"Detected personalization_data_type='persona data' from query")
                        elif any(phrase in request.query.lower() for phrase in ["audience data", "use audience", "with audience"]):
                            personalization_data_type = "audience data"
                            parameters["personalization_data_type"] = "audience data"
                            logger.info(f"Detected personalization_data_type='audience data' from query")
                    ...
                    if not personalization_data_type:
                        ...
                        if conversation_id and request.user_id and self.mongodb:
                            try:
                                if hasattr(self.mongodb, 'get_context_value'):
                                    data_type = await self.mongodb.get_context_value(request.user_id, conversation_id, "personalization_data_type")
                                elif hasattr(self.mongodb, 'get_conversation'):
                                    conversation = await self.mongodb.get_conversation(conversation_id=conversation_id)
                                    if conversation and "context" in conversation and "personalization_data_type" in conversation["context"]:
                                        data_type = conversation["context"]["personalization_data_type"]
                            except Exception as e:
                                logger.error(f"Error retrieving personalization_data_type from context: {str(e)}")
                        
                        if data_type:
                            parameters["personalization_data_type"] = data_type
                            personalization_data_type = data_type
                            logger.info(f"Retrieved personalization_data_type={data_type} from conversation context")
                
                # If we still don't have the personalization data type, ask user to mention it in their query
                if not personalization_data_type:
                    logger.info(f"Personalization data type not specified for {tool_selection.tool_name}. Asking user to mention it in query.")
                ...
                if conversation_id and request.user_id and self.mongodb and personalization_data_type:
                    try:
                        if hasattr(self.mongodb, 'update_conversation_context'):
                            await self.mongodb.update_conversation_context(
                                conversation_id=conversation_id, 
                                context_updates={"personalization_data_type": personalization_data_type}
                            )
                        elif hasattr(self.mongodb, 'update_query_context'):
                            await self.mongodb.update_query_context(
                                user_id=request.user_id,
                                conversation_id=conversation_id,
                                new_params={"personalization_data_type": personalization_data_type},
                                expected_keys=None
                            )
                    except Exception as e:
                        logger.error(f"Error storing personalization_data_type in context: {str(e)}")
                ...
                if personalization_data_type.lower() == "audience data":
                    ...
                    if not parameters.get("audience_id"):
                        ...
                        if conversation_id and request.user_id and self.mongodb:
                            try:
                                if hasattr(self.mongodb, 'get_context_value'):
                                    audience_id = await self.mongodb.get_context_value(request.user_id, conversation_id, "audience_id")
                                elif hasattr(self.mongodb, 'get_conversation'):
                                    conversation = await self.mongodb.get_conversation(conversation_id=conversation_id)
                                    if conversation and "context" in conversation and "audience_id" in conversation["context"]:
                                        audience_id = conversation["context"]["audience_id"]
                            except Exception as e:
                                logger.error(f"Error retrieving audience_id from context: {str(e)}")
                        
                        if audience_id:
                            parameters["audience_id"] = audience_id
                            logger.info(f"Retrieved audience_id={audience_id} from conversation context")
                        else:
                            logger.info("Audience ID not provided for persona creation. Requesting audience_id first.")
                ...
                elif personalization_data_type.lower() == "model data" or personalization_data_type.lower() == "persona data":
                    ...
                    if not parameters.get("model_id"):
                        ...
                        if conversation_id and request.user_id and self.mongodb:
                            try:
                                if hasattr(self.mongodb, 'get_context_value'):
                                    model_id = await self.mongodb.get_context_value(request.user_id, conversation_id, "model_id")
                                elif hasattr(self.mongodb, 'get_conversation'):
                                    conversation = await self.mongodb.get_conversation(conversation_id=conversation_id)
                                    if conversation and "context" in conversation and "model_id" in conversation["context"]:
                                        model_id = conversation["context"]["model_id"]
                            except Exception as e:
                                logger.error(f"Error retrieving model_id from context: {str(e)}")
                        
                        if model_id:
                            parameters["model_id"] = model_id
                            logger.info(f"Retrieved model_id={model_id} from conversation context")
                        else:
                            logger.info("Model ID not provided for persona creation. Requesting model_id first.")
            ...
            if tool_selection.tool_name:
                ...
                tool_result = await self.execute_tool(tool_selection.tool_name, parameters)
                
                # Special handling for persona_summary tool
                if tool_selection.tool_name == "persona_summary" and tool_result.status == "success":
                    # Extract persona name from the result if available
                    persona_name = ""
                    model_id = parameters.get("model_id", "")
                    
                    if tool_result.data and isinstance(tool_result.data, dict) and "persona_name" in tool_result.data:
                        persona_name = tool_result.data["persona_name"]
                    elif tool_result.data and isinstance(tool_result.data, dict) and "output_persona_json" in tool_result.data:
                        persona_json = tool_result.data["output_persona_json"]
                        if isinstance(persona_json, dict) and "persona_name" in persona_json:
                            persona_name = persona_json["persona_name"]
                    
                    # Store the persona_name in conversation context for future use
                    if conversation_id and request.user_id and self.mongodb and persona_name:
                        # Update conversation context with persona info
                        if hasattr(self.mongodb, 'update_conversation_context'):
                            await self.mongodb.update_conversation_context(
                                conversation_id=conversation_id, 
                                context_updates={"persona_name": persona_name, "model_id": model_id}
                            )
                        elif hasattr(self.mongodb, 'update_query_context'):
                            await self.mongodb.update_query_context(
                                user_id=request.user_id,
                                conversation_id=conversation_id,
                                new_params={"persona_name": persona_name, "model_id": model_id},
                                expected_keys=None
                            )
                        ...
                        try:
                            ...
                            if hasattr(self.mongodb, 'save_persona_summary'):
                                ...
                                if tool_result.data and isinstance(tool_result.data, dict):
                                    output_data = tool_result.data.get("output_persona_json", {})
                                    if isinstance(output_data, dict):
                                        logger.info(f"Updating persona summary with conversation_id: {conversation_id}")
                            ...
                            logger.error(f"Error updating persona summary with conversation_id: {str(e)}", exc_info=True)
                ...
                if tool_selection.tool_name == "journy_tool" and tool_result.status == "success" and parameters.get("action") == "create_journey":
                    ...
                    if conversation_id and request.user_id and self.mongodb and (journey_id or mongo_id):
                        # Log the journey IDs we're storing
                        logger.info(f"Storing journey IDs in context: journey_id={journey_id}, mongo_id={mongo_id}")
                ...
                needed_input = None
                if hasattr(tool_result, 'required_inputs') and tool_result.required_inputs:
