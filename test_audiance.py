...
def create_personalized_email(
    ...
    for filter_set in filter_sets:
        ...
        for i, column_name in enumerate(filter_set.get("column_names", [])):
            ...
            if (len(filter_conditions) == 1 and isinstance(filter_conditions[0], list) and 
                len(filter_conditions[0]) > 0 and isinstance(filter_conditions[0][0], list)):
                filter_conditions = filter_conditions[0]
            ...
            for condition in filter_conditions:
                ...
                if operator == "equal" and user_data[column_name] != threshold:
                    ...
                    is_match = False
                elif operator == "greater" and not (isinstance(user_data[column_name], (int, float)) and user_data[column_name] > threshold):
                    is_match = False
                elif operator == "less" and not (isinstance(user_data[column_name], (int, float)) and user_data[column_name] < threshold):
                    is_match = False
                elif operator == "contains" and not (isinstance(user_data[column_name], str) and threshold.lower() in user_data[column_name].lower()):
                    is_match = False
    ...
    try:
        ...
        segments_text = "\n".join([f"- {segment}" for segment in matching_segments]) if matching_segments else "No specific segments matched"
...
def create_generalized_personalized_email(
    ...
    try:
        ...
        filter_criteria_text = "\n".join([f"- {criteria}" for criteria in all_filter_criteria]) if all_filter_criteria else "No specific criteria defined"
...
def demo_standalone_email_generation():
    ...
    print(f"Personalization Factors: {email_content.get('personalization_factors', [])}")
...
5,Carol Davis,Sometown,35,allergies,2023-08-12"""
    ...
    print(f"Subject Line Template: {template.get('subject_line_template', 'No template')}")
    print(f"Greeting Template: {template.get('greeting_template', 'No template')}")
    print(f"Body Content Template: {template.get('body_content_template', 'No template')}")
    print(f"Call to Action Template: {template.get('call_to_action_template', 'No template')}")
...
# if __name__ == "__main__":
    ...
    print(f"Subject Line Template: {email_template.get('subject_line_template', 'No template')}")
    print(f"Greeting Template: {email_template.get('greeting_template', 'No template')}")
    print(f"Body Content Template: {email_template.get('body_content_template', 'No template')}")
    print(f"Call to Action Template: {email_template.get('call_to_action_template', 'No template')}")
